...
/**
 * FtpController class must be derived from Controller class
 */
class FtpController : public Controller {
     ...
     * construct with unique controller name to differentiate it from others
		 */
		FtpController():Controller("Ftp"){}

		/**
		 * register Ftp controller
     *
     ...
     * handler method will only call if user is authenticated before with login credentials.
		 */
		void boot( void ){
			this->m_route_handler->register_route( "/Ftp", [&]() { this->handleFtpRoute(); }, AUTH_MIDDLEWARE );
		}

    /**
		 * build and send Ftp page to client
		 */
    void handleFtpRoute( void ) {

      #ifdef EW_SERIAL_LOG
      Logln(F("Handling Ftp route"));
      #endif
